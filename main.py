...
target_words = ["travel deals"]


def find_spam_by_name(search_query):
    authors = []
    for submission in reddit.subreddit("all").search(search_query, sort="new", limit=100):
...
if __name__ == "__main__":
    ...
    while True:
        ...
        current_search_query = random.choice(target_words)
        print("> run {}".format(current_search_query))
        spam_content = []
        trashy_users = {}
        ...
        for author in smelly_authors:
            ...
            try:
                ...
                if trashy_score >= 0.5 and sub_count > 1:#TRASH SCORE OVER .5 -> ANSWER
...
At least {}% out of the {} submissions from /u/{} appear to be {} spam. 

Don't let spam take over Reddit! Throw it out!

*Bee bop*""".format(round(trashy_users[spam_user][0]*100,2), trashy_users[spam_user][1], spam_user, current_search_query)
                try:
                    if DEBUG_MODE:
                    ...
                    if link not in already_posted:
                        ...
                        print("We've posted to {} and now we need to sleep for 12 minute".format(link))
